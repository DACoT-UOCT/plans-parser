...
def setup_args():
    ...
    parser.add_argument(
        "diffdb",
        type=str,
        help="database that is updated with new programs and plans data",
    )
    parser.add_argument(
        "proddb",
        type=str,
        help="database that contains all the production data and we want to update",
    )
...
def build_patches(new_data, current_data):
    ...
    for jid in diff_juncs.keys():
        if jid not in curr_juncs:
            log.warning(
                "For some reason jid={} exists in the new database, but not in the production database".format(
                    jid
                )
            )
            continue
...
def save_new_data_junctions(new_data):
    ...
    for operation in new_data:
        ...
        juncobj.id = mongo_id
        change = ChangeSet(
            apply_to_id=jid, apply_to=juncobj, changes=patch, message="Automatic Update"
        )
...
def save_new_data_otus(new_data):
    ...
    for operation in new_data:
        ...
        otuobj.id = mongo_id
        change = ChangeSet(
            apply_to_id=oid, apply_to=otuobj, changes=patch, message="Automatic Update"
        )
...
def patch_db(args):
    ...
    log.info(
        "We have {} Junction and {} OTU patches to process".format(
            len(patches[0]), len(patches[1])
        )
    )
