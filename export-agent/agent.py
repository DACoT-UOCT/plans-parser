...
def build_schedules(from_env=False):
    ...
    with open(fout, 'w') as fp:
        json.dump(failed, fp)
    log.info('Written {} failed plans, {} failed program lines for L table, {} failed program lines for S table and {} failed program lines for D table in {} file'
        .format(len(failed['failed_plans']), len(failed['failed_program_tables']['L']), len(failed['failed_program_tables']['S']),
        len(failed['failed_program_tables']['D']), fout))

def read_params():
    parser = argparse.ArgumentParser(description='Extract schedules from the UOCT control system')
