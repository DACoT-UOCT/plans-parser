...
class SchedulesExtractor:
    def __init__(self, host, user, passwd, debug=False, logger=None):
        ...
        self.__re_ansi_escape = re.compile(
            r"\x1B(?:[@-Z\\-_]|[0-9]|\[[0-?]*[ -/]*[@-~])|\r|\n"
        )
    ...
    def build_schedules(self):
        ...
        failed_dict = self.__build_failed_results_dict(
            failed_plans,
            [failed_week_programs, failed_saturday_programs, failed_sunday_programs],
        )
        ...
        for p in programs:
            if p[0][0] == "A":
                ...
                if self.__debug_enabled:
                    self.__log_print(
                        "{} is not in schedules (no plans for this junction?) day: {} parsed: {}".format(
                            p[0], day, p
                        )
                    )
        return schedules
    ...
    def __parse_programs(self, table_code):
        self.__log_print(
            "Starting call to __parse_programs with table_code={}".format(table_code)
        )
        self.__login()
        ...
        if self.__debug_enabled:
            self.__log_print(
                "From a total of {} plans, we parsed {} and {} have problems".format(
                    len(system_programs), len(programs), len(fail)
                )
            )
    ...
    def __parse_plans(self, junction):
        ...
        for plan in system_plans:
            ...
            if not ok:
                ...
                if self.__debug_enabled:
                    self.__log_print(
                        "PARSE FAILED: {}".format(bytes(clean_plan, "ascii"))
                    )
            else:
                success += 1
        ...
        if self.__debug_enabled:
            self.__log_print(
                "From a total of {} plans, we parsed {}, {} have problems and {} were ignored".format(
                    len(system_plans), success, len(fail), ignored
                )
            )
        return plans, fail
